  ...
  if(length(l.args) > 0){
    if("start.params.life" %in% names(l.args))
      if(length(l.args$start.params.life)>0)
        stop("The start parameters given for covariates are not needed because this is a no covariate model!", call. = FALSE, immediate. = TRUE)
    ...
    if("names.cov.life" %in% names(l.args))
      if(length(l.args$names.cov.life)>0)
        stop("The covariate names given are not needed because this is a no covariate model!", call. = FALSE, immediate. = TRUE)
    ...
    if("reg.lambdas" %in% names(l.args))
      if(length(l.args$reg.lambdas)>0)
        stop("The regularization lambdas are not needed because this is a no covariate model!", call. = FALSE, immediate. = TRUE)
    if("start.params.constr" %in% names(l.args))
      if(length(l.args$start.params.constr) > 0)
        stop("The given parameters to constrain and their start parameters are not needed because this is a no covariate model!", call. = FALSE, immediate. = TRUE)
