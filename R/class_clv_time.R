...
clv.time.set.sample.periods <- function(clv.time, tp.first.transaction, tp.last.transaction, user.estimation.end, user.observation.end){

  tp.estimation.start <- tp.first.transaction

  if(is.null(user.observation.end)){
    tp.observation.end <- tp.last.transaction
  }else{
    tp.observation.end <- clv.time.convert.user.input.to.timepoint(
      clv.time=clv.time,
      user.timepoint=user.observation.end)

    # Observation end may not be before last transaction
    if(tp.observation.end < tp.last.transaction){
      stop("The given observation.end may not be before the last recorded transaction!")
    }
  }
  ...
  if(!is.null(user.estimation.end)){
    ...
    tp.estimation.end  <- tp.observation.end
