...
namespace clv{
  ...
  arma::vec vec_hyp2F1(const arma::vec& vA, const arma::vec& vB, const arma::vec& vC, const arma::vec& vX){
    // arma::vec::const_iterator it_a = vA.begin(), it_b=vB.begin(), it_c=vC.begin(), it_x=vX.begin();
    // arma::vec::const_iterator it_a_end = vA.end();
    //
    arma::vec vRes(vA);
    // arma::vec::const_iterator it_res = vRes.begin();
    //
    // while(it_a != it_a_end){
    //
    //   it_res++;
    ...
    for(arma::uword i = 0; i<n; i++)
      vRes(i) = cephes::hypergeom2F1(vA(i), vB(i), vC(i), vX(i));
...
arma::vec vec_x_hyp1F1(const double a, const double b, const arma::vec& vX){
  ...
  for(arma::uword i = 0; i<n; i++)
    vRes(i) = cephes::hypergeom1F1(a, b, vX(i));
