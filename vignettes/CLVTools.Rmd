...
Data for time-varying contextual factors requires a time-series of contextual factor values for every customer. I.e. if the time-varying contextual factors are allowed to change every week, a value for every customer for every week is required. Note that all contextual factors are required to use the same time intervals for the time-series. In the example of the apparel retailer we use information on direct marketing (`Marekting`) as time-varying contextual factors. Additionally, we add gender as time-invariant contextual factors. Note that the data structure of invariant contextual factors needs to be aligned with the structure of time-varying contextual factors. Use `data("apparelDynCov")` command to load the time-varying contextual factors. In this example `Cov.Date` indicates the first day of a weekly contextual factor time-interval.
