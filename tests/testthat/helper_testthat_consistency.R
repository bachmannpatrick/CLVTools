fct.testthat.consistency.cov.data.0.cov.params.insignificant <- function(m.static){
  test_that("Cov params are insignificant", {
    expect_true(all(coef(summary(m.static))[c("life.Gender", "life.Channel", "trans.Gender", "trans.Channel"), 4] > 0.1))
  })
}

fct.testthat.consistency.cov.data.0.model.params.nearly.same <- function(m.nocov, m.static, param.names){
  test_that("Model parameters are nearly the same", {
    expect_true(all.equal(coef(m.nocov), coef(m.static)[param.names], tolerance = 0.05))
  })
}
...
fct.testthat.consistency.cov.params.0.predict.same <- function(m.nocov, m.static, m.dyncov, has.DERT = TRUE, has.dyncov = TRUE){
...
fct.testthat.consistency.cov.params.0.plot.same <- function(m.nocov, m.static, m.dyncov, has.dyncov = TRUE){
